class GestaoRelatorios {
    ...
    async carregarDados() {
        // Alimentos
        await fetch('../../backend/controllers/AlimentoController.php?action=listar')
            .then(res => res.json())
            .then(data => { this.alimentos = data; });
    ...
    async mudarAba(aba) {
        ...
        // Recarrega os dados do backend antes de renderizar a aba
        await this.carregarDados();
    ...
    renderizarGraficoEstoque() {
        ...
        const cores = this.alimentos.map(alimento => {
            ...
            if (parseFloat(alimento.quantidade) <= parseFloat(alimento.estoque_minimo ?? alimento.estoqueMinimo)) return '#FFCE56'; // Baixo
    ...
    // Corrige clicks das abas para chamar mudarAba
    configurarAbaClicks() {
    ...
    // Exporta de Relatórios (PDF e Excel)
