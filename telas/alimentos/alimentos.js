...
function renderizarTabela(alimentos) {
  ...
  alimentosPagina.forEach((alimento) => {
    ...
    linha.innerHTML = `
      ...
      <td>
        <button type="button" class="btn-editar" title="Editar" onclick='editarAlimento(${JSON.stringify(alimento).replace(/"/g, "&quot;")})'><i class="fas fa-edit"></i></button>
        <button type="button" class="btn-consumo" title="Registrar Consumo" onclick='abrirModalConsumo(${JSON.stringify(alimento).replace(/"/g, "&quot;")})'><i class="fas fa-minus-circle"></i></button>
        <button type="button" class="btn-excluir" title="Excluir" onclick='deletarAlimento(${alimento.id_alimento})'><i class="fas fa-trash-alt"></i></button>
      </td>
    `;
